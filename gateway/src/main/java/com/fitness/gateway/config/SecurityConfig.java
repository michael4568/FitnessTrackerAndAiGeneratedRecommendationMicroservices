...
public class SecurityConfig {
    ...
    public SecurityWebFilterChain securityWebFilterChain(ServerHttpSecurity http) {
        http
                ...
                // NOTE: not a @Bean — registered here only so it runs after JWT auth is
                // resolved
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        // Explicit origin required when allowCredentials is true — wildcard (*) is
        // forbidden by spec
        corsConfig.setAllowedOrigins(List.of("http://localhost:5173"));
        corsConfig.setMaxAge(3600L);
        corsConfig.setAllowedMethods(List.of("GET", "POST", "PUT", "DELETE", "OPTIONS", "PATCH"));
        corsConfig.setAllowedHeaders(List.of("Authorization", "Content-Type", "X-Gemini-API-Key", "X-User-Id",
